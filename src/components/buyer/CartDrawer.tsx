...
import { 
  ...
  MdCloudUpload, MdAccountBalanceWallet, MdCheckCircle, 
  MdLocationOn, MdPayment, MdArrowForward, MdInfoOutline,
...
export default function CartDrawer() {
  ...
  const [isInitialLoading, setIsInitialLoading] = useState(false) // For tab switching
  const [isProcessing, setIsProcessing] = useState(false) // For major actions (Sync/Checkout)
  const [updatingId, setUpdatingId] = useState<string | null>(null) // For specific item QTY changes
  ...
  const refreshData = useCallback(async (showLoader = false) => {
    ...
    try {
      ...
      if (bRes.data.status) setBillingAddress(bRes.data.billing_address || '')
    } catch (e) {
  ...
  useEffect(() => {
    if (cartOpen) {
      ...
      // Auto-switch to vendoor if local is empty
  ...
  const handleSyncToVendoor = async () => {
    if (!isAuthenticated()) return router.push('/auth?mode=signin')
    setIsProcessing(true)
    const tid = toast.loading("Moving items to Vendoor...")
    try {
      for (const item of localItems) {
        await cartApi.add(item.product_uuid, item.quantity)
      }
      await refreshData()
      clearCart()
      ...
      toast.error("Partial sync failure", { id: tid })
    } finally {
  ...
  const handleVendoorUpdate = async (uuid: string, qty: number) => {
    ...
    setUpdatingId(uuid)
    try {
      const res = await cartApi.update(uuid, qty)
      if (res.data.status) await refreshData()
    } finally {
      setUpdatingId(null)
    }
  }
  ...
  const saveAddress = async () => {
    setIsProcessing(true)
    try {
      await billingApi.save(billingAddress)
    ...
    <>
      ...
      <div className={`fixed right-0 top-0 bottom-0 w-full md:w-[450px] bg-[#F4F7F9] z-[501] flex flex-col shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.32,0,0.07,1)] ${cartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="bg-white p-5 border-b flex items-center justify-between shrink-0">
          ...
          <button onClick={closeCart} className="p-2 hover:bg-gray-100 rounded-full transition-all active:scale-75"><MdClose size={28} /></button>
        </div>

        {/* Tab Nav */}
        <div className="flex bg-white p-2 border-b gap-1 shrink-0">
          {(['local', 'vendoor', 'checkout'] as const).map((t) => (
            <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-3 rounded-xl text-[11px] font-[900] uppercase tracking-tighter transition-all ${activeTab === t ? 'bg-[#F85606] text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50'}`}>
              {t}
            </button>
          ))}
        </div>
                ...
                <div className="space-y-4">
                  {localItems.length > 0 ? (
                    <>
                      ...
                      <button onClick={handleSyncToVendoor} className="w-full h-14 bg-[#F85606] text-white rounded-2xl font-black text-[14px] shadow-lg shadow-orange-100 flex items-center justify-center gap-2 mt-6">
                    ...
                    <>
                      {vendoorItems.map(item => (
                        <ItemRow 
                          ...
                          onUpdate={handleVendoorUpdate} 
                          onRemove={handleVendoorRemove} 
                        />
                      ))}
                      <button onClick={() => setActiveTab('checkout')} className="w-full h-14 bg-black text-white rounded-2xl font-black text-[14px] flex items-center justify-center gap-2 mt-4">
                ...
                <div className="space-y-6">
                  <div className="bg-white p-5 rounded-2xl border shadow-sm">
                    <h3 className="text-[14px] font-black mb-4 flex items-center gap-2"><MdLocationOn className="text-[#F85606]" /> DELIVERY ADDRESS</h3>
                      <div className="space-y-3">
                        <textarea className="w-full bg-gray-50 border rounded-xl p-4 text-[13px] min-h-[100px] outline-none focus:ring-2 ring-orange-500/20" placeholder="Hostel name, room number, and phone..." value={billingAddress} onChange={e => setBillingAddress(e.target.value)} />
                        <button onClick={saveAddress} className="w-full bg-black text-white py-3 rounded-xl font-bold text-[12px]">Update Address (Delivery)</button>
                      </div>
                  ...
                  <div className="bg-white p-5 rounded-2xl border shadow-sm">
                    ...
                    {billingAddress && vendoorItems.length > 0 ? (
                      <PaystackButton {...paystackConfig} className="w-full h-14 bg-[#00853D] text-white rounded-2xl font-black text-[16px] shadow-xl shadow-green-100 hover:scale-[1.02] transition-transform" />
                    ) : (
