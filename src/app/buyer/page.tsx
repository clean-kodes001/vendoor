...
// Assuming your SLIDES array now has an 'img' property (a, b, or c)
const SLIDES = [
  { img: 'b', tag: 'Limited', title: 'Tech Essentials', sub: 'Upgrade your study setup with pro gear.', cat: 'electronics' },
]
...
export default function BuyerHomePage() {
  ...
  function ImageBanner({ heroIdx, goSlide, SLIDES }: any) {
    return (
      <div className="relative group px-4 py-2">
        {/* Container: Apple "Card" Style */}
        <div className="relative h-[180px] sm:h-[240px] w-full overflow-hidden rounded-[24px] bg-gray-100 border border-gray-200/50">
          ...
          >
            {SLIDES.map((s: any, i: number) => (
              ...
              >
                <img
                  src={`https://vendoor.ng/uploads/${s.img}.jpeg`}
                  alt="Banner"
          ...
          {/* Minimal Navigation Arrows: Only visible on hover */}
          <button
            onClick={() => goSlide((heroIdx - 1 + SLIDES.length) % SLIDES.length)}
            className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-gray-900 shadow-sm z-20"
          >
          ...
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-gray-900 shadow-sm z-20"
          >
            ...
            {SLIDES.map((_: any, i: number) => (
              <div
                ...
                className={cn(
                  ...
                  heroIdx === i ? "w-4 bg-white shadow-sm" : "w-1 bg-white/50"
                )}
              />
            ))}
          </div>
  ...
  return (
    <div className="page-enter">
      ...
      <ImageBanner heroIdx={heroIdx} SLIDES={SLIDES} goSlide={goSlide}
      />
      {/* Spotlight Vendors */}
      {vendors.length > 0 && (
